...
- **Auth:** HTTP Basic. Credentials `jwnam` / `<password>` are the real Komga account; never hardcode them in source — they are entered at the login screen at runtime.
...
curl -s -o /dev/null -w "%{http_code}\n" -u jwnam:<password> https://macmini.tail993f0c.ts.net/api/v1/users/me  # 200
```
